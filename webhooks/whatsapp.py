...
async def handle_message(from_phone: str, body_text: str):
    """Background task: creates its own DB session to avoid closed-session issues."""
    try:
    ...
    async with async_session() as db:
        try:
            print(f"[Bot] Processing message from {from_phone}: '{body_text}'")
            ...
            for change in entry.get("changes", []):
                ...
                for message in messages:
                    ...
                    if from_phone and text_body:
                        print(f"[Webhook] Message from {from_phone}: '{text_body}'")
